...
void creature_appliquer_force(Creature *creature, float temps,int input_gauche, int input_droite) {
    float force_horizontale = 0;
    if (input_gauche) {
        force_horizontale -= FORCE_DEPLACEMENT;
    }
    if (input_droite) {
        force_horizontale += FORCE_DEPLACEMENT;
    }
    for (int i = 0; i < creature->nb_masses; i++) {
        ...
        if (masse->y >= SOL_Y) { //Deplacement uniquement si la masse touche le sol sinon on se deplace pas dans les air en gros sans appuie
            masse->forcex = force_horizontale; // ajoute le deplacment a la masse en question
        } else {
            masse->forcex = 0;
        }
        masse->forcey = masse->poid * GRAVITY;
        if (masse->y > SOL_Y) {
            masse->y = SOL_Y;
            masse->vitessey = 0;        // le sol arrete le mouvement vertical
            creature->energie -= ENERGIE_PERTE_SOL;
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    int input_gauche = 0; // Deplacement 
    int input_droite = 0;
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_KEYDOWN) { //presse vert le bas
                if (e.key.keysym.sym == SDLK_LEFT) {
                    input_gauche = 1;
                } else if (e.key.keysym.sym == SDLK_RIGHT) {
                    input_droite = 1;
                }
            } else if (e.type == SDL_KEYUP) { // relache le bouton
                if (e.key.keysym.sym == SDLK_LEFT) {
                    input_gauche = 0;
                } else if (e.key.keysym.sym == SDLK_RIGHT) {
                    input_droite = 0;
        ...
        creature_appliquer_force(&c, temps,input_gauche, input_droite);
        c.temps_survie += dt;
